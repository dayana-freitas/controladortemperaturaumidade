...
#define dmux_enable     0 // DMUX
#define dmux_seletor    1 
#define data1           2 // Entrada de Dados
#define shift1          3 // Registrador de Saída
#define store1          4 // Registrador de Deslocamento
#define data2           5 
#define shift2          6  
#define store2          7  
#define data3           8
...
void setup()
{
	...
	pinMode(dmux_enable,  OUTPUT); // Saídas via DMUX 
	pinMode(dmux_seletor, OUTPUT);
...
void loop()
{
	...
	{	//Loop executa até que o usuário pressione o botão de função		
		temp = map(analogRead(sensor_temp), 0, 1023, 0, 100); // Mapeia os dados do sensor de temperatura para valores entre 0°C e 100°C
		umid = map(analogRead(sensor_umid), 0, 1023, 0, 100); // Mapeia os dados do sensor de umidade para valores entre 0% e 100%
...
int contador(int inputA2, int inputA3, int inputA4){
	...
	if (digitalRead(inputA2) == 0 && digitalRead(inputA3) == 0 && digitalRead(inputA4) == 0) {delay(300); cont = 0;}
	if (digitalRead(inputA2) == 0 && digitalRead(inputA3) == 0 && digitalRead(inputA4) == 1) {delay(300); cont = 1;}
	if (digitalRead(inputA2) == 0 && digitalRead(inputA3) == 1 && digitalRead(inputA4) == 0) {delay(300); cont = 2;}
	if (digitalRead(inputA2) == 0 && digitalRead(inputA3) == 1 && digitalRead(inputA4) == 1) {delay(300); cont = 3;}
	if (digitalRead(inputA2) == 1 && digitalRead(inputA3) == 0 && digitalRead(inputA4) == 0) {delay(300); cont = 4;}
...
void controlarSaidas(int umid, int temp) {
	...
	//Controlado pelo dmux
    if (temp >= temp_max) {
		digitalWrite(dmux_enable, LOW);
		digitalWrite(dmux_seletor, LOW);
    }
    if (temp <= temp_min) {
		digitalWrite(dmux_enable, LOW);
		digitalWrite(dmux_seletor, HIGH);
    }
	if (temp > temp_min && temp < temp_max) {
		digitalWrite(dmux_enable, HIGH);
    } 	 	
}
